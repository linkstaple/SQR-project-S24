...
class _Group:
    ...
    def list_users(self, user_id):
        groups = Database.fetch(
            '''select groups_users.group_id as id,
    ...
    def get_members(self, group_id):
        members = Database.fetch(
            '''select groups_users.user_id as id,
    ...
    def add_transaction(self, group_id, doer_id, lander_id, payer_ids, amount):
        ...
        for payer_id in payer_ids:
            self._increase_balance_not_commit(
                group_id, payer_id, -amount / len(payer_ids))
        SplitHistory.add_not_commit(
            group_id, doer_id, lander_id, payer_ids, amount)
        Database.commit()
