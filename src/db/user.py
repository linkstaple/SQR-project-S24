...
class _User:
    ...
    def register_user(self, username, hashed_password):
        Database.execute_and_commit("insert into users (username, password) values ($1, $2)",
                                    username, hashed_password)

    def get_by_credentials(self, username, hashed_password):
        resp = Database.fetch("select id, username from users where username = $1 and password = $2",
                              username, hashed_password)
